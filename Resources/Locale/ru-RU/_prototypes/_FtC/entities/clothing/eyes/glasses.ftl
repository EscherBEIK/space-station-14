ent-ClothingEyesSalesman = цветные очки
    .desc = Пара очков с уникальными цветными линзами. На оправе надпись "Лучший продавец 1997".
ent-ClothingEyesBinoclardLenses = бинокулярные линзы
    .desc = Показывает, что вы умеете и пришивать лацкан и центрировать заднее вентиляционное отверстие.
ent-ClothingEyesGlassesBlueShield = очки офицера «синий щит»
    .desc = Инновационные очки синего оттенка с большим количеством незаявленных улучшений. Хочется верить, что это не перекрашенные солнцезащитные очки службы безопасности. 
ent-ClothingEyesGlassesWelding = Сварочные Очки
    .desc = Это сварочные очки. Это небольшой предмет. Защищает глаза от сварщиков.
ent-ClothingEyesSunGlassesHudSkill = солнечные очки умений
    .desc = Очки с встроенным HUD умений, показывающим историю трудовой деятельности ближайших членов экипажа NT.
